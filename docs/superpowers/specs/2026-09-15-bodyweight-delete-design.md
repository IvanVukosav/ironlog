...
Match the app's existing delete-control convention, not the primary-action bracket style: `deleteSet` (`ExerciseCard.module.css`) and `historyEntryDelete` (`Stats.module.css`) both render as plain dim text, `color-accent` on hover, no brackets, no background/border. Add a new `.bwDeleteButton` class in `Calendar.module.css` following that exact styling, placed in the modal footer opposite `bwSaveButton` (`[ Spremi ]` stays bracketed and unchanged — it's the primary action). Keeping delete visually quiet avoids it reading as equally-weighted with Save, and matches every other delete affordance already in the app.

**Correction (post-implementation, caught by user):** the sibling controls use an "✕" glyph, but this modal already has its own "X" close button in the top-right corner (`bwModalClose`) — using "✕" for delete too made it look like a second, misplaced close button rather than a distinct destructive action. This modal's delete control uses the text label "Obriši" instead, keeping the same dim→accent-hover styling but avoiding the glyph collision. `deleteSet`/`historyEntryDelete` keep their "✕"/"X" glyphs unchanged — they have no competing close button nearby, so no collision exists there.
